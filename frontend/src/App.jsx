...
const HistoryItem = ({ item }) => {
    ...
    return (
        <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700 w-full text-left transition hover:border-indigo-500/50">
            <div className="flex justify-between items-start gap-4">
                <p className="text-gray-300 italic flex-1 break-words">"{item.ticket_text}"</p>
            ...
            {item.status === 'COMPLETE' && item.result ? (
                <div className="mt-3 pt-3 border-t border-gray-700 flex flex-wrap gap-y-2 justify-between items-center">
                    ...
                    <a
                        ...
                        download
                        className="flex items-center text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-1 px-3 rounded-lg transition duration-300"
                    >
                        <DownloadIcon />
             ...
             {item.status === 'FAILED' && (
                <div className="mt-2 text-sm text-red-400 break-words">
                    Error: {item.error_message || 'An unknown error occurred.'}
                </div>
            )}
        </div>
    );
...
export default function App() {
    ...
    // Fetch history on load and set up polling
    useEffect(() => {
    ...
    const hasCompletedAnalyses = useMemo(() => 
            ...
            <div className="w-full max-w-4xl text-center">
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-xl border border-gray-700">
                        ...
                        <form onSubmit={handleTextSubmit}>
                            <textarea
                                ...
                                placeholder="Paste a single customer ticket here..."
                                className="w-full h-32 p-3 bg-gray-900 border border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none transition"
                                disabled={isLoading}
                            />
                            <button type="submit" className="mt-4 w-full flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition" disabled={isLoading}>
                    ...
                    <div className="bg-gray-800/40 p-6 rounded-xl border border-gray-700">
                        ...
                        <div className="flex flex-col items-center justify-center h-full">
                             ...
                             <p className="text-xs text-gray-500 mt-2 text-center">
                                Please upload a .txt file where each line is a separate customer feedback entry.
                             </p>
                             {file && <p className="text-sm text-gray-400 mt-2">Selected: {file.name}</p>}
                            <button onClick={handleFileSubmit} className="mt-4 w-full flex items-center justify-center bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition" disabled={!file || isLoading}>
                ...
                <div className="w-full">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {/* ADDED: Container for the action buttons */}
                        <div className="flex items-center gap-x-2">
                            {hasCompletedAnalyses && (
                                <a href="/api/analyses/download-all" download="full_analysis_history.txt" className="flex items-center text-sm bg-blue-600/80 hover:bg-blue-700 text-white font-semibold py-1 px-3 rounded-lg transition">
                                    <DownloadAllIcon/> Download All
                                </a>
                            )}
                            {history.length > 0 && (
                                <button onClick={handleClearHistory} className="flex items-center text-sm bg-red-600/80 hover:bg-red-700 disabled:bg-gray-600 text-white font-semibold py-1 px-3 rounded-lg transition" disabled={isLoading}>
                    ...
                    <div className="space-y-4">
                        {history.length > 0 ? (
                            ...
                            <div className="text-gray-500 p-8 border-2 border-dashed border-gray-700 rounded-lg">
                                <p>No analyses yet.</p>
                                <p>Submit a ticket or upload a file to get started.</p>
                            </div>
                        )}
                    </div>
                </div>
